...
// specified address.
func PayToAddrScript(addr zenutil.Address) ([]byte, error) {
...
func PayToAddrReplayOutScript(addr zenutil.Address, blockHash []byte, blockHeight int64) ([]byte, error) {
	const nilAddrErrStr = "unable to generate payment script for nil address"
	switch addr := addr.(type) {
	case *zenutil.AddressPubKeyHash:
		if addr == nil {
...
func MultiSigScript(pubkeys []*zenutil.AddressPubKey, nrequired int) ([]byte, error) {
...
func ExtractPkScriptAddrs(pkScript []byte, chainParams *chaincfg.Params) (ScriptClass, []zenutil.Address, int, error) {
	var addrs []zenutil.Address
	...
	case PubKeyHashTy, PubKeyHashReplayOutTy:
		...
		requiredSigs = 1
		addr, err := zenutil.NewAddressPubKeyHash(pops[2].data,
			chainParams)
		if err == nil {
	...
	case PubKeyTy:
		...
		requiredSigs = 1
		addr, err := zenutil.NewAddressPubKey(pops[0].data, chainParams)
		if err == nil {
			addrs = append(addrs, addr)
		}
	...
	case ScriptHashTy, ScriptHashReplayOutTy:
		...
		requiredSigs = 1
		addr, err := zenutil.NewAddressScriptHashFromHash(pops[1].data,
			chainParams)
		if err == nil {
			addrs = append(addrs, addr)
		}
	...
	case MultiSigTy:
		...
		addrs = make([]zenutil.Address, 0, numPubKeys)
		for i := 0; i < numPubKeys; i++ {
			addr, err := zenutil.NewAddressPubKey(pops[i+1].data,
				chainParams)
			if err == nil {
				addrs = append(addrs, addr)
			}
