...
func newAddressPubKey(serializedPubKey []byte) zenutil.Address {
	addr, err := zenutil.NewAddressPubKey(serializedPubKey,
...
// test source code.
func newAddressPubKeyHash(pkHash []byte) zenutil.Address {
	addr, err := zenutil.NewAddressPubKeyHash(pkHash, &chaincfg.MainNetParams)
	if err != nil {
...
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressScriptHash(scriptHash []byte) zenutil.Address {
	addr, err := zenutil.NewAddressScriptHashFromHash(scriptHash,
		&chaincfg.MainNetParams)
	if err != nil {
...
func TestExtractPkScriptAddrs(t *testing.T) {
	...
	for i, test := range tests {
		...
		if err != nil {
			t.Error("error in ExtractPkScriptAddrs")
		}
...
func TestPayToAddrScript(t *testing.T) {
	...
	p2pkhMain, err := zenutil.NewAddressPubKeyHash(hexToBytes("e34cce70c86"+
	...
	p2shMain, _ := zenutil.NewAddressScriptHashFromHash(hexToBytes("e8c300"+
	...
	p2pkCompressedMain, err := zenutil.NewAddressPubKey(hexToBytes("02192d"+
	...
	p2pkCompressed2Main, err := zenutil.NewAddressPubKey(hexToBytes("03b0b"+
	...
	p2pkUncompressedMain, err := zenutil.NewAddressPubKey(hexToBytes("0411"+
	...
	tests := []struct {
		...
		{(*zenutil.AddressPubKeyHash)(nil), "", errUnsupportedAddress},
		{(*zenutil.AddressScriptHash)(nil), "", errUnsupportedAddress},
		{(*zenutil.AddressPubKey)(nil), "", errUnsupportedAddress},
...
func TestMultiSigScript(t *testing.T) {
	...
	tests := []struct {
		keys      []*zenutil.AddressPubKey
		nrequired int
		expected  string
		err       error
	}{
		{
			[]*zenutil.AddressPubKey{
				p2pkCompressedMain,
				p2pkCompressed2Main,
			},
			1,
