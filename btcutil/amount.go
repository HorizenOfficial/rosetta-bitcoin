...
// monetary amount.
const (
	AmountMegaZEN  AmountUnit = 6
	AmountKiloZEN  AmountUnit = 3
	AmountZEN      AmountUnit = 0
	AmountMilliZEN AmountUnit = -3
	AmountMicroZEN AmountUnit = -6
	AmountZentoshi AmountUnit = -8
)

// String returns the unit as a string.  For recognized units, the SI
// prefix is used, or "Zentoshi" for the base unit.  For all unrecognized
// units, "1eN ZEN" is returned, where N is the AmountUnit.
func (u AmountUnit) String() string {
	...
	case AmountZentoshi:
		return "Zentoshi"
	default:
		return "1e" + strconv.FormatInt(int64(u), 10) + " ZEN"
	}
}
...
//
// NewAmount is for specifically for converting ZEN to Zentoshi.
// For creating a new Amount with an int64 value which denotes a quantity of Satoshi,
// do a simple type conversion from type int64 to Amount.
// See GoDoc for example: http://godoc.org/github.com/HorizenOfficial/rosetta-zen/btcutil#example-Amount
func NewAmount(f float64) (Amount, error) {
	...
	case math.IsInf(f, 1):
		fallthrough
	case math.IsInf(f, -1):
		return 0, errors.New("invalid zen amount")
	}

	return round(f * ZentoshiPerZen), nil
}
...
func (a Amount) ToZEN() float64 {
	return a.ToUnit(AmountZEN)
}
