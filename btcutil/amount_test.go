...
func TestAmountUnitConversions(t *testing.T) {
	...
	for _, test := range tests {
		...
		// Verify that Amount.ToZEN works as advertised.
		f1 := test.amount.ToUnit(AmountZEN)
		f2 := test.amount.ToZEN()
		if f1 != f2 {
			t.Errorf("%v: ToZEN does not match ToUnit(AmountZEN): %v != %v", test.name, f1, f2)
		}

		// Verify that Amount.String works as advertised.
		s1 := test.amount.Format(AmountZEN)
...
func TestAmountMulF64(t *testing.T) {
	tests := []struct {
		...
		{
			name: "Multiply 0.1 ZEN by 2",
			amt:  100e5, // 0.1 ZEN
			mul:  2,
			res:  200e5, // 0.2 ZEN
		},
		{
			name: "Multiply 0.2 ZEN by 0.02",
			amt:  200e5, // 0.2 ZEN
			mul:  1.02,
			res:  204e5, // 0.204 ZEN
		},
